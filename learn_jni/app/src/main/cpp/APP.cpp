...
namespace toolkit {
    ...
    std::string APP::get_apk_signature(JNIEnv *env, jclass clazz, jobject obj) {
        jclass clz = env->GetObjectClass(obj);
        jmethodID pm_method = env->GetMethodID(clz, "getPackageManager",
                                               "()Landroid/content/pm/PackageManager;");
        // 获得 PackageManager 对象
        jobject pkg_obj = env->CallObjectMethod(obj, pm_method);
        // 获得 PackageManager 类
        jclass pm_clz = env->GetObjectClass(pkg_obj);
        jmethodID pkg_info_method = env->GetMethodID(pm_clz, "getPackageInfo",
        ...
        // 获得签名数组属性的字段
        jfieldID signatures_field = env->GetFieldID(pkg_info_clz, "signatures",
        ...
        jsize size = env->GetArrayLength(signatures_arr);
        // 获取数组的第 0 个签名对象
        jobject signature_obj = env->GetObjectArrayElement(signatures_arr, 0);
        jclass signature_clz = env->GetObjectClass(signature_obj);
        jmethodID to_char_method = env->GetMethodID(signature_clz, "toCharsString",
                                                    "()Ljava/lang/String;");
        // 得到签名
        jstring signature = static_cast<jstring>(env->CallObjectMethod(signature_obj,
                                                                       to_char_method));
